...
<div class="card">
    ...
    {{ trans('cruds.password.edit') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.passwords.update", [$password->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('pass') ? 'has-error' : '' }}">
                ...
                <input type="password" id="pass" name="pass" class="form-control" value="{{ old('pass', isset($password) ? $password->pass : '') }}"required>
                @if($errors->has('pass'))
                    <em class="invalid-feedback">
                        {{ $errors->first('pass') }}
                    </em>
            ...
            <div class="form-group {{ $errors->has('tags') ? 'has-error' : '' }}">
                ...
                <select name="tags[]" id="tags" class="form-control select2" multiple="multiple">
                    @foreach($tags as $id => $name)
                        <option value="{{ $id }}" {{ (in_array($id, old('tag', [])) || isset($password) && $password->tags->contains($id)) ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <em class="invalid-feedback">
            ...
            <div class="form-group {{ $errors->has('users') ? 'has-error' : '' }}">
                ...
                <select name="users[]" id="users" class="form-control select2" multiple="multiple">
                    @foreach($users as $id => $name)
                        <option value="{{ $id }}"  {{ (in_array($id, old('user', [])) || isset($password) && $password->users->contains($id)) ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
                    <em class="invalid-feedback">
